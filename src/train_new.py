...
if __name__ == '__main__':
    ...
    with open("./tmpmodel/args.json", 'w') as f:
    ...
    for epoch in range(args.epochs):
        ...
        history['loss_train'].append(outputs[0])
        history['acc_train'].append(outputs[1])
        history['loss_val'].append(outputs[2])
        history['acc_val'].append(outputs[3])
        history['cur_lr'].append(outputs[4])
        history['s_time'].append(sampling_t)
        history['t_time'].append(outputs[5])
        history['v_time'].append(outputs[6])

        pd.DataFrame(history).to_csv(f"runs/run_history-dataset_{args.dataset}-type_{args.type}")
